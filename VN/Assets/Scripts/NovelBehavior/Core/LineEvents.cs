...
public class LineEvents : MonoBehaviour {
	...
	public static void HandleEvent( string _event, LineArchitect.LINE.SEGMENT segment ) {
		if (_event.Contains("(")) {
			//get all actions delimitted by a comma
			string[] actions = _event.Split(' ');
			for (int i = 0; i < actions.Length; i++) {
				NovelController._instance.HandleAction(actions[i]);
			}
			return;
		}
	...
	static void EVENT_TxtSpd( string data, LineArchitect.LINE.SEGMENT seg ) {
